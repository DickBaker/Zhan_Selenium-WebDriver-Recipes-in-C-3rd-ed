...
// need to install 
namespace SeleniumRecipesCSharp.ch08_select;

[TestClass]
public class Ch08SelectTest
{
    ...
    {
        SelectElement select = new(driver.FindElement(By.Id("car_make_select")));
    ...
    [TestMethod]
    public void TestSelectOptionBySendKeys()
    {
        driver.FindElement(By.Id("car_make_select")).SendKeys("honda"); // select "Honda (Japan)"
        Thread.Sleep(500); // important, needs to add some waiting here
        driver.FindElement(By.Id("car_make_select")).SendKeys("Audi"); // select "Audi (Germany)"
    }
    ...
    {
        SelectElement select = new(driver.FindElement(By.Name("test_framework")));
        select.SelectByText("Selenium");
        select.SelectByText("RWebSpec");
