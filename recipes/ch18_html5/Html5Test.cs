...
public class Ch18Html5Test
{
    ...
    [TestMethod]
    public void TestEmailField() => driver.FindElement(By.Id("email")).SendKeys("test@wisely.com");
    ...
    public void TestOnChangeForTextField()
    {
        ...
        driver.ExecuteScript("$('#person_name_textbox').trigger('change')");
    ...
    public static void ClearChosen(string elemId)
    {
        Thread.Sleep(500);
    ...
    public static void SelectChosenByLabel(string elemId, string label)
    {
    ...
    [TestMethod]
    public void TestFakeGeolocationWithJavaScript()
    {
        driver.Navigate().GoToUrl("https://whenwise.agileway.net");
        driver.FindElement(By.Id("tab-location")).Click();

        const double latitude = -33.8571197;
        const double longitude = 151.2138464;
        driver.ExecuteScript("window.navigator.geolocation.getCurrentPosition=function(success){; var position = {'coords' : {'latitude': '" + latitude + "','longitude': '" + longitude + "'}}; success(position);}");

        driver.FindElement(By.Id("get-current-location-link")).Click();
        Thread.Sleep(2500); // wait for location to populate
        Assert.IsTrue(driver.FindElement(By.Id("location")).GetAttribute("value").Equals("2 Circular Quay E, Sydney NSW 2000, Australia"));
    }
    /*
            [TestMethod]
            public void TestSaveChartAsPng() {
