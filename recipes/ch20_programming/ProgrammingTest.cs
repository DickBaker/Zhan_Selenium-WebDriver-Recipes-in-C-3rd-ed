...
[TestClass]
public class Ch20ProgrammingTest
{
    ...
    {
        string filePath = TestHelper.ScriptDir() + "/testdata/in.xml";
    ...
    /**
     * Refer CSVReader.cs in the same project
     */
    [TestMethod]
    public void TestDataDrivenCSV()
    {
        // Iterate each row in the CSV file, use data for test scripts
        string csvFilePath = Path.GetFullPath(TestHelper.ScriptDir() + "/testdata/users.csv");

        IEnumerable<List<string>> data = CSVReader.FromFile(csvFilePath);

        foreach (List<string> nextLine in data)
        {
    ...
    {
        driver.Url = "https://agileway.com.au/demo/assertion";
    ...
    public void TestVerifySearchResultsOrder()
    {
        ...
        Thread.Sleep(500); // wait JS to load
        ...
        List<string> sortedProductNames = new(productNames);
    ...
    public void TestExtractDynamicTextUsingRegex()
    {
        ...
        string couponText = driver.FindElement(By.Id("details")).Text;
        Console.WriteLine(couponText);
